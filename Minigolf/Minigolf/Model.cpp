...
Mesh* Model::processMesh(ID3D11Device* device, aiMatrix4x4 transformation, aiMesh * mesh)
{
	...
	for (size_t i = 0; i < mesh->mNumVertices; ++i)
	{
		...
		{
			vertex._textureCoords.x = (float)mesh->mTextureCoords[0][i].x * 100.0f; //Jag antar att diffuse texture ligger på index 0.
			vertex._textureCoords.y = (float)mesh->mTextureCoords[0][i].y* 100.0f; //Så när vi vill ha normal map ligger den troligen på ett annat index.
		}
