...
XMVECTOR DynamicObject::calcGliding(float deltaSeconds, XMVECTOR acceleration)
{
	...
	float yg = 0.25; //Ska ej finnas här
	_angularVelocity = XMVectorSet(0.0f , (5 * yg * 9.82 *deltaSeconds) / (2 * 0.0214) - XMVectorGetY(_angularVelocity), 0.0f, 0.0f); //Update angularVelocity //Ersätt radius, getY så länge det bara är backspinn. Annars måste vi räkna beloppet.
